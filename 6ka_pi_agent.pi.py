...
TEMP_STATUS_PATH = os.getenv("PI_AGENT_TEMP_STATUS_PATH", "/home/djay/projects/order_notify/switchbot_temp_status.json")
...
def read_json_file(path):
    try:
        ...
        if updated_at:
            try:
                age_seconds = int((datetime.now() - datetime.fromisoformat(updated_at)).total_seconds())
            except Exception:
                age_seconds = None
        return {
            "ok": True,
...
def service_state(name, heartbeat_path=None):
    code, active, _ = run(["systemctl", "is-active", name])
    _, enabled, _ = run(["systemctl", "is-enabled", name])
    _, status, _ = run(["systemctl", "show", name, "--property=MainPID,ExecMainStartTimestamp,ActiveState,SubState,NRestarts", "--no-page"])
    detail = {}
    ...
    payload = {
        ...
        "active_state": detail.get("ActiveState"),
        "sub_state": detail.get("SubState"),
        "started_at": detail.get("ExecMainStartTimestamp"),
        "restart_count": detail.get("NRestarts"),
    }
    if heartbeat_path:
        payload["heartbeat"] = read_json_file(heartbeat_path)
    return payload


def fetch_player_health():
    try:
        with urlrequest.urlopen(PLAYER_LOCAL_API + "/health", timeout=3) as response:
...
def parse_temperature_log():
    heartbeat = read_json_file(TEMP_STATUS_PATH)
    if heartbeat.get("ok") and heartbeat.get("payload"):
...
def status_payload():
    return {
        ...
        "services": {
            ...
            "temperature": service_state(TEMP_SERVICE, TEMP_STATUS_PATH),
        },
        "player": fetch_player_health(),
        "audio": audio_status(),
